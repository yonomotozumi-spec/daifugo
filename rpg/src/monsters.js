/**
 * 魔物の絵。canvas に アニメ（セル画）ふうで描く。
 *
 * 描きかたの決まり:
 *   ・太い輪郭線でシルエットを立てる
 *   ・影は グラデーションではなく 平らな一段の色で入れる（セル画の陰）
 *   ・目は大きく、ハイライトを 2 つ入れて 生きた感じにする
...
/**
 * セル画ふうの塗り。path を描く関数と、その大まかな囲みを渡す。
 * 塗り → 下半分に影 → 左上にハイライト → 輪郭線、の順。
 */
function cel(ctx, pathFn, box, color, { lineWidth, shadeAt = 0.56, gloss = true, outline = true } = {}) {
  const line = shade(color, -0.62);
  ctx.save();
  pathFn();
  ctx.fillStyle = color;
  ctx.fill();
  ...
  ctx.beginPath();
  ctx.moveTo(box.x - box.w, box.y + box.h * shadeAt + box.h * 0.12);
  ctx.lineTo(box.x + box.w * 2, box.y + box.h * shadeAt - box.h * 0.06);
  ...
  if (gloss) {
    ...
    ctx.beginPath();
    ctx.ellipse(box.x + box.w * 0.34, box.y + box.h * 0.26, box.w * 0.2, box.h * 0.13, -0.5, 0, TAU);
    ctx.fill();
  }
  ctx.restore();

  if (outline) {
    pathFn();                 // 影を描いたぶん 形が変わっているので 引き直してから ふちどる
...
/**
 * アニメ風の目。大きな白目・色つきの虹彩・ハイライト 2 点。
 * mood: 'cute' | 'fierce' | 'calm'、glow を渡すと 光る目になる。
 */
function animeEyes(ctx, { x, y, gap, w, h, iris = '#2b3a55', mood = 'cute', glow = null, lw }) {
  const draw = (ex, flip) => {
    if (glow) {
      ctx.save();
      ...
      ctx.fillStyle = glow;
      ctx.beginPath();
      ctx.ellipse(ex, y, w * 0.6, h * 0.5, 0, 0, TAU);
      ctx.fill();
      ctx.restore();
      return;
    }
    ...
    ctx.fill();
    ctx.strokeStyle = 'rgba(20,22,34,0.85)';
    ctx.lineWidth = lw * 0.7;
    ctx.stroke();
    // 虹彩と瞳
    ctx.fillStyle = iris;
    ctx.beginPath();
    ctx.ellipse(ex + flip * w * 0.12, y + h * 0.08, w * 0.62, h * 0.72, 0, 0, TAU);
    ctx.fill();
    ctx.fillStyle = '#14161f';
    ctx.beginPath();
    ctx.ellipse(ex + flip * w * 0.12, y + h * 0.12, w * 0.3, h * 0.42, 0, 0, TAU);
    ctx.fill();
    // ハイライト 2 点
    ctx.fillStyle = '#ffffff';
    ctx.beginPath();
    ctx.ellipse(ex - w * 0.28, y - h * 0.34, w * 0.24, h * 0.22, -0.4, 0, TAU);
    ctx.fill();
    ctx.beginPath();
  ...
  if (mood === 'fierce') {
    ctx.strokeStyle = 'rgba(18,20,30,0.9)';
    ctx.lineWidth = lw * 1.4;
    ctx.lineCap = 'round';
    ctx.beginPath();
    ctx.moveTo(x - gap / 2 - w * 1.1, y - h * 1.5);
    ctx.lineTo(x - gap / 2 + w * 0.8, y - h * 0.7);
    ctx.moveTo(x + gap / 2 + w * 1.1, y - h * 1.5);
    ctx.lineTo(x + gap / 2 - w * 0.8, y - h * 0.7);
    ctx.stroke();
  }
...
const PAINTERS = {
  ...
  /** 四つ足のけもの。 */
  beast(ctx, p, s, t) {
    const bob = Math.sin(t * 2.6) * s * 0.012;
    const lw = s * 0.028;
    const bodyW = s * 0.34;
    const bodyH = s * 0.2;
    const cx = -s * 0.04;
    ...
    // しっぽ
    ctx.save();
    ctx.strokeStyle = shade(p.body, -0.45);
    ctx.lineWidth = s * (p.thinTail ? 0.028 : 0.06);
    ctx.lineCap = 'round';
    ctx.beginPath();
    ctx.moveTo(cx - bodyW * 0.9, cy - bodyH * 0.1);
    ctx.quadraticCurveTo(cx - bodyW * 1.7, cy - bodyH * (1.2 + Math.sin(t * 4) * 0.4), cx - bodyW * 1.5, cy - bodyH * 1.9);
    ctx.stroke();
    ctx.restore();

    // あし
    for (const [lx, delay] of [[-0.6, 0], [0.55, 1.6]]) {
      const swing = Math.sin(t * 4 + delay) * s * 0.012;
      celEllipse(ctx, cx + bodyW * lx + swing, cy + bodyH * 1.05, s * 0.055, s * 0.075, shade(p.body, -0.2), lw, { gloss: false });
    }

    celEllipse(ctx, cx, cy, bodyW, bodyH, p.body, lw);

    // あたま
    const hx = cx + bodyW * 0.95;
    ...
    if (p.mane) {
      cel(ctx, () => {
        ...
        for (let i = 0; i < 9; i++) {
          const a2 = (i / 9) * TAU;
          const r = s * (i % 2 ? 0.19 : 0.25);
          const px = hx + Math.cos(a2) * r;
          const py = hy + Math.sin(a2) * r;
          if (i === 0) ctx.moveTo(px, py);
          else ctx.lineTo(px, py);
        }
        ctx.closePath();
      }, { x: hx - s * 0.25, y: hy - s * 0.25, w: s * 0.5, h: s * 0.5 }, shade(p.body, -0.22), { lineWidth: lw, gloss: false });
    }
    celEllipse(ctx, hx, hy, s * 0.17, s * 0.15, p.body, lw);
    ...
    if (p.ears !== false) {
      for (const side of [-1, 1]) {
        cel(ctx, () => {
          ctx.beginPath();
          ctx.moveTo(hx + side * s * 0.05 - s * 0.05, hy - s * 0.09);
          ctx.lineTo(hx + side * s * 0.09 + s * 0.02, hy - s * 0.26);
          ctx.lineTo(hx + side * s * 0.13, hy - s * 0.06);
          ctx.closePath();
        }, { x: hx - s * 0.12, y: hy - s * 0.26, w: s * 0.3, h: s * 0.24 }, p.body, { lineWidth: lw, gloss: false });
      }
    }
    // せなかのトゲ
    if (p.spikes) {
      for (let i = -1; i <= 1; i++) {
        cel(ctx, () => {
          ctx.beginPath();
          ctx.moveTo(cx + i * bodyW * 0.42 - s * 0.035, cy - bodyH * 0.85);
          ctx.lineTo(cx + i * bodyW * 0.42, cy - bodyH * 1.6);
          ctx.lineTo(cx + i * bodyW * 0.42 + s * 0.035, cy - bodyH * 0.85);
          ctx.closePath();
        }, { x: cx - s * 0.1, y: cy - bodyH * 1.6, w: s * 0.2, h: bodyH }, shade(p.body, -0.25), { lineWidth: lw * 0.8, gloss: false });
      }
    }

    // はな先
    celEllipse(ctx, hx + s * 0.13, hy + s * 0.05, s * 0.07, s * 0.055, shade(p.body, 0.12), lw * 0.8, { gloss: false });
    ctx.fillStyle = '#2b2b33';
    ctx.beginPath();
    ctx.ellipse(hx + s * 0.19, hy + s * 0.03, s * 0.02, s * 0.016, 0, 0, TAU);
    ctx.fill();

    animeEyes(ctx, { x: hx + s * 0.02, y: hy - s * 0.02, gap: s * 0.13, w: s * 0.05, h: s * 0.055, iris: p.eye, mood: p.mood || 'fierce', lw: s * 0.02 });
    mouth(ctx, { x: hx + s * 0.13, y: hy + s * 0.1, w: s * 0.11, h: s * 0.05, kind: p.mouth || 'fang', lw: s * 0.02 });
  },
  ...
  humanoid(ctx, p, s, t) {
    ...
    cel(ctx, () => {
      ctx.beginPath();
      ctx.moveTo(-s * 0.2, bob + s * 0.34);
      ctx.quadraticCurveTo(-s * 0.24, bob - s * 0.08, -s * 0.13, bob - s * 0.12);
      ctx.lineTo(s * 0.13, bob - s * 0.12);
      ctx.quadraticCurveTo(s * 0.24, bob - s * 0.08, s * 0.2, bob + s * 0.34);
      ctx.closePath();
    }, { x: -s * 0.24, y: bob - s * 0.12, w: s * 0.48, h: s * 0.46 }, p.cloth, { lineWidth: lw });

    // うで
    for (const side of [-1, 1]) {
      celEllipse(ctx, side * s * 0.22, bob + s * 0.08, s * 0.06, s * 0.11, p.skin, lw * 0.9, { gloss: false, rotate: side * 0.3 });
    }
    ...
    // あたま
    const hy = bob - s * 0.26;
    celEllipse(ctx, 0, hy, s * 0.17, s * 0.16, p.skin, lw);
